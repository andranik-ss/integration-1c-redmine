...
Процедура КоманднаяПанельСписокЗадачОбновитьСписок(Кнопка)
	...
	Отборы = Отборы + "&include=journals";
...
Процедура ОставитьКомментарийКЗадачеНажатие(Элемент)
...
Процедура ДобавитьЗаписьОЗатраченомВремениНажатие(Элемент)
...
Процедура ОткрытьФормуЗадачиДляРедактирования(мНомерЗадачи) Экспорт
	...
	ДанныеРесурса = ПолучитьДанныеРесурсаИзТрекера("issues/"+Формат(мНомерЗадачи,"ЧГ=0")+"", ОтчетОВыполнении, Отборы);
...
Функция СформироватьДеревоДопПолейТрекера()
	...
	Если ЗагруженныеДанные.Получить(Ресурс) = Неопределено Или ПовторноеПолучаниеДанных Тогда
		...
		ОтчетОВыполнении = СформироватьСтруктуруОтчетаОВыполнении();
		ДанныеРесурса = ПолучитьДанныеРесурсаИзТрекера(Ресурс, ОтчетОВыполнении, СформироватьПараметрыЗапросаПолученияДанныхСТрекера(ЭтаФорма.ЭлементыФормы));
...
Процедура ЗаполнитьСписокВыбораИсполнителейОтборы()
	...
	Если Не ВсеРеквизитыЗаполнены(Новый Структура(ОбязательныеПоля() + ", Проект"), "Сформировать список выбора исполнителей",, ЭтаФорма) Тогда
		Возврат;
	КонецЕсли;
...
Процедура УстановитьВидимость()
	...
	ЭлементыФормы.РамкаГруппыНастраиваимыеПоля.Видимость	= тНастраиваемыеПоля.Количество() > 0;
...
Процедура СписокЗадачПриАктивизацииСтроки(Элемент)
	...
	Если ЭлементыФормы.ДополнительнаяПанель.Страницы.История.Раскрыта Тогда
		ВывестиИсториюИзмененийЗадачи(Формат(ТекСтрокаСписка.Идентификатор,"ЧГ=0"));
	КонецЕсли;
...
Процедура ВывестиИсториюИзмененийЗадачи(мНомерЗадачи)
	...
	Если ДанныеОЗадаче = Неопределено Или Не ДанныеОЗадаче["issue"].Свойство("journals") Тогда
		...
		ДанныеОЗадаче = ПолучитьДанныеРесурсаИзТрекера("issues/"+мНомерЗадачи, ОтчетОВыполнении, Отборы);
