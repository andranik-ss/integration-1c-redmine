...
Функция СоздатьЗадачуВТрекере(ОтчетоВыполнении) Экспорт
	...
	Если Вложения.Количество() <> 0 Тогда
		Для Каждого Вложение Из Вложения Цикл
			Результат = ОтправитьЗапросВТрекер(СформироватьСтруктуруЗапросаВТрекер("POST", "uploads", Вложение.ПолноеИмяФайла), ОтчетоВыполнении);
			Вложение.Токен = Результат.upload.token;
		КонецЦикла;
	КонецЕсли;
	
	// Подготовить файл к отправке в трекер
	ФайлКОтправке = ПреобразоватьСтруктуруВ_JSON(СформироватьСтруктуруЗадачи(), ОтчетоВыполнении);
	
	Результат = ОтправитьЗапросВТрекер(СформироватьСтруктуруЗапросаВТрекер("POST", "issues", ФайлКОтправке), ОтчетоВыполнении);
...
Функция ОбновитьЗадачуВТрекере(ОтчетоВыполнении) Экспорт
	...
	// Подготовить файл к отправке в трекер
	ФайлКОтправке = ПреобразоватьСтруктуруВ_JSON(СформироватьСтруктуруЗадачи(Истина), ОтчетоВыполнении);
	
	Результат = ОтправитьЗапросВТрекер(СформироватьСтруктуруЗапросаВТрекер("PUT", "issues/"+Формат(ЗадачаНомер, "ЧГ=0"), ФайлКОтправке), ОтчетоВыполнении);
	...
	Если ЗадачаЗатраченноеВремя <> 0  Тогда
		ФайлКОтправке = ПреобразоватьСтруктуруВ_JSON(СформироватьСтрутуруЗаписиЗатраченногоВремени(), ОтчетоВыполнении);
		
		Результат = ОтправитьЗапросВТрекер(СформироватьСтруктуруЗапросаВТрекер("POST", "time_entries", ФайлКОтправке), ОтчетоВыполнении);
	КонецЕсли;
...
Функция УдалитьЗадачуИзТрекера(НомерЗадачи, ОтчетоВыполнении) Экспорт
	...
	Если ДанныеСвязей <> Неопределено Тогда  // Необходимо удалить связи
		Для Каждого Связь Из ДанныеСвязей["relations"] Цикл
			ОтправитьЗапросВТрекер(СформироватьСтруктуруЗапросаВТрекер("DELETE", "/relations/"+Связь.id), ОтчетоВыполнении);		
			Если ОтчетоВыполнении.ЕстьОшибки Тогда
				Возврат Ложь;		
			КонецЕсли;	
		КонецЦикла;	
	...
	//2. удаление задачи
	ОтправитьЗапросВТрекер(СформироватьСтруктуруЗапросаВТрекер("DELETE", "/issues/"+Формат(НомерЗадачи, "ЧГ=0")), ОтчетоВыполнении);		
	Если ОтчетоВыполнении.ЕстьОшибки Тогда
		Возврат Ложь;		
	КонецЕсли;	
...
Функция ЗадачаУжеДобавленаВТрекер(СтруктураДанных, ОтчетоВыполнении) Экспорт
...
Функция Инициализация(Отборы = Ложь, Проект = Неопределено, ОтчетОВыполнении = Неопределено) Экспорт
...
Функция СформироватьПараметрыЗапросаПолученияДанныхСТрекера(Элементы) Экспорт
	Отборы = "";
...
Функция СформироватьСтруктуруЗадачи(Редактирование=Ложь)
	...
	ПоляЗадачи.Вставить("parent_issue_id",    	  ?(ЗадачаНомерРродительскойЗадачи = 0, Неопределено, ЗадачаНомерРродительскойЗадачи)); 	
	...
	Если Редактирование И ЗначениеЗаполнено(ЗадачаПримечание) Тогда
		ПоляЗадачи.Вставить("notes", ЗадачаПримечание); 
	КонецЕсли;
...
Функция ОтправитьЗапросВТрекер(СтруктураЗапроса, ОтчетоВыполнении)
	...
	СтруктураЗапроса.АдресЗапроса = СтруктураЗапроса.АдресЗапроса + ОбязательныйПараметр + СтруктураЗапроса.Параметры;
	
	Попытка
		Если СтруктураЗапроса.Метод = "GET" Тогда
			HTTPОтвет = мHTTPСоединение.Получить(Новый HTTPЗапрос(СтруктураЗапроса.АдресЗапроса));
		Иначе
...
Функция ОбработатьРезультатОтвета(HTTPОтвет, ОтчетОВыполнении)
	...
	Если HTTPОтвет <> Неопределено Тогда
		...
		Если Результат.Свойство("error") Тогда
			ОтчетоВыполнении.ЕстьОшибки = Истина;
			ОтчетоВыполнении.КодСостояния = HTTPОтвет.КодСостояния;
			ОтчетоВыполнении.ТекстОшибки = "Ошибка выполнения запроса. Код состояния:" + HTTPОтвет.КодСостояния + Символы.ПС + Результат["error"];
			Возврат Результат;
		КонецЕсли;
