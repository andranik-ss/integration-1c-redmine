...
Перем ОтчетОВыполнении Экспорт;
...
Функция СоздатьЗадачуВТрекере() Экспорт
	...
	ФайлКОтправке = ПреобразоватьСтруктуруВ_JSON(СформироватьСтруктуруЗадачи());
	
	Результат = ОтправитьЗапросВТрекер(СформироватьСтруктуруЗапросаВТрекер("POST", "issues", ФайлКОтправке));
...
Функция ОбновитьЗадачуВТрекере() Экспорт
	...
	// Подготовить файл к отправке в трекер
	ФайлКОтправке = ПреобразоватьСтруктуруВ_JSON(СформироватьСтруктуруЗадачи(Истина));
	
	Результат = ОтправитьЗапросВТрекер(СформироватьСтруктуруЗапросаВТрекер("PUT", "issues/"+Формат(ЗадачаНомер, "ЧГ=0"), ФайлКОтправке));
	...
	Если ЗадачаЗатраченноеВремя <> 0  Тогда
		ФайлКОтправке = ПреобразоватьСтруктуруВ_JSON(СформироватьСтрутуруЗаписиЗатраченногоВремени());
		
		Результат = ОтправитьЗапросВТрекер(СформироватьСтруктуруЗапросаВТрекер("POST", "time_entries", ФайлКОтправке));
	КонецЕсли;
...
Функция ПреобразоватьСтруктуруВ_JSON(СтруктураДанных)
...
ПределПолучаемыхДанных = 25; // Предел получаемых данных с сервера, по умолчанию.
