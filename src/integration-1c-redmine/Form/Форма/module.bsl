...
Процедура ПроверкаСоединенияНажатие(Элемент)
	...
	Если НЕ ОтчетОВыполнении.ЕстьОшибки Тогда
		...
		Предупреждение("Соединение с трекером успешно выполнено!",, "ВНИМАНИЕ!!!");
...
Функция СформироватьДерево(ВидДерева)
	...
	Если ВидДерева = "issues" Тогда
		Дерево.Колонки.Добавить("ПроектЗадача",           ОбщегоНазначения.ПолучитьОписаниеТиповСтроки(100), "Проект/Задача", 100);
		Дерево.Колонки.Добавить("Идентификатор",          ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(5,0), 	"Номер задачи", 10);
		//Дерево.Колонки.Добавить("Тема",                   ОбщегоНазначения.ПолучитьОписаниеТиповСтроки(150), "Тема");
		//Дерево.Колонки.Добавить("Описание",               ОбщегоНазначения.ПолучитьОписаниеТиповСтроки(1000),"Описание");
		//Дерево.Колонки.Добавить("Статус",                 ОбщегоНазначения.ПолучитьОписаниеТиповСтроки(100),, 20);
		//Дерево.Колонки.Добавить("Приоритет",              ОбщегоНазначения.ПолучитьОписаниеТиповСтроки(100),, 20);
		//Дерево.Колонки.Добавить("Трекер",                 ОбщегоНазначения.ПолучитьОписаниеТиповСтроки(100),, 20);
		//Дерево.Колонки.Добавить("Исполнитель",            ОбщегоНазначения.ПолучитьОписаниеТиповСтроки(100),, 30);
		//Дерево.Колонки.Добавить("ОценкаВремени",          ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(10,2),, 10);
		//Дерево.Колонки.Добавить("ПроцентВыполнения",      ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(10,2),, 10);
		//Дерево.Колонки.Добавить("ДополнительныеПоля",   ,, 10);
		//Дерево.Колонки.Добавить("ДатаНачала",             ОбщегоНазначения.ПолучитьОписаниеТиповСтроки(100),, 20);
		//Дерево.Колонки.Добавить("ДатаВыполнения",         ОбщегоНазначения.ПолучитьОписаниеТиповСтроки(100),, 20);
		Дерево.Колонки.Добавить("ИдентификаторПроекта",   ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(5,0), 	"Ид. проекта", 10);
	ИначеЕсли ВидДерева = "projects" Тогда
		Дерево.Колонки.Добавить("НазваниеПроекта",        ОбщегоНазначения.ПолучитьОписаниеТиповСтроки(100),	"Название проета", 30);
		Дерево.Колонки.Добавить("Описание",               ОбщегоНазначения.ПолучитьОписаниеТиповСтроки(1000), 	"Описание", 50);
		Дерево.Колонки.Добавить("Идентификатор",          ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(5,0), 	"Идентификатор", 10);
	ИначеЕсли ВидДерева = "users" Тогда
		Дерево.Колонки.Добавить("Идентификатор",          ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(5,0), 	"Идентификатор", 10);
		Дерево.Колонки.Добавить("Имя",                    ОбщегоНазначения.ПолучитьОписаниеТиповСтроки(100), 	"Имя", 20);
		Дерево.Колонки.Добавить("Фамилия",                ОбщегоНазначения.ПолучитьОписаниеТиповСтроки(100), 	"Фамилия", 20);
		Дерево.Колонки.Добавить("Логин",                  ОбщегоНазначения.ПолучитьОписаниеТиповСтроки(100), 	"Логин", 20);
		Дерево.Колонки.Добавить("Почта",                  ОбщегоНазначения.ПолучитьОписаниеТиповСтроки(100), 	"Почта", 20);
		Дерево.Колонки.Добавить("ДатаСоздания",           ОбщегоНазначения.ПолучитьОписаниеТиповСтроки(100), 	"Создан", 10);
		Дерево.Колонки.Добавить("ДатаАвторизации",        ОбщегоНазначения.ПолучитьОписаниеТиповСтроки(100), 	"Авторизировался", 10);
	ИначеЕсли ВидДерева = "custom_fields" Тогда
		Дерево.Колонки.Добавить("Идентификатор",          ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(5,0), 	"Идентификатор", 10);
		Дерево.Колонки.Добавить("ИмяПоля",                ОбщегоНазначения.ПолучитьОписаниеТиповСтроки(100), 	"Имя поля", 20);
		Дерево.Колонки.Добавить("ТипКастомизации",        ОбщегоНазначения.ПолучитьОписаниеТиповСтроки(100), 	"Тип кастомизации", 20);
		Дерево.Колонки.Добавить("ФорматПоля",             ОбщегоНазначения.ПолучитьОписаниеТиповСтроки(100), 	"Формат поля", 20);
		Дерево.Колонки.Добавить("МинДлина",               ОбщегоНазначения.ПолучитьОписаниеТиповСтроки(100), 	"Мин. длина", 20);
		Дерево.Колонки.Добавить("МаксДлина",              ОбщегоНазначения.ПолучитьОписаниеТиповСтроки(100), 	"Макс. длина", 10);
		Дерево.Колонки.Добавить("ЗначениеПоУмолчанию",    ОбщегоНазначения.ПолучитьОписаниеТиповСтроки(100), 	"По умолчанию", 10);
		Дерево.Колонки.Добавить("Видимость",              ОбщегоНазначения.ПолучитьОписаниеТиповСтроки(100), 	"Видимость", 10);
		Дерево.Колонки.Добавить("Обязательный",           ОбщегоНазначения.ПолучитьОписаниеТиповСтроки(100), 	"Обязательное", 10);
		Дерево.Колонки.Добавить("Фильтруется",            ОбщегоНазначения.ПолучитьОписаниеТиповСтроки(100), 	"Фильтруется", 10);
		Дерево.Колонки.Добавить("УчаствуетВПоиске",       ОбщегоНазначения.ПолучитьОписаниеТиповСтроки(100), 	"Участвует в поиске", 10);
	КонецЕсли;
