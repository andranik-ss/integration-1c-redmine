...
Перем ПоляТрекераПоУмолчанию Экспорт;
...
Функция СоздатьЗадачуВТрекере(ОтчетоВыполнении) Экспорт
	...
	Если Вложения.Количество() <> 0 Тогда
		Для Каждого Вложение Из Вложения Цикл
			Результат = ОтправитьДанныеВТрекер("POST", "uploads", ОтчетоВыполнении, Вложение.ПолноеИмяФайла);
			Вложение.Токен = Результат.upload.token;
		КонецЦикла;
	КонецЕсли;
	
	// Подготовить файл к отправке в трекер
	ФайлКОтправке = СформироватьФайлКОтправкеJSON(СформироватьСтрутуруДанных(), ОтчетоВыполнении);
	
	Результат = ОтправитьДанныеВТрекер("POST", "issues", ОтчетоВыполнении, ФайлКОтправке);
	
	Возврат Результат;
КонецФункции

Функция ОбновитьЗадачуВТрекере(ОтчетоВыполнении) Экспорт
	...
	// Подготовить файл к отправке в трекер
	ФайлКОтправке = СформироватьФайлКОтправкеJSON(СформироватьСтрутуруДанных(Истина), ОтчетоВыполнении);
	
	Результат = ОтправитьДанныеВТрекер("PUT", "issues/"+Формат(ЗадачаНомер, "ЧГ=0"), ОтчетоВыполнении, ФайлКОтправке);
	...
	Если ЗадачаЗатраченноеВремя <> 0  Тогда
		ФайлКОтправке = СформироватьФайлКОтправкеJSON(СформироватьСтрутуруДанныхВремя(), ОтчетоВыполнении);
		
		Результат = ОтправитьДанныеВТрекер("POST", "time_entries", ОтчетоВыполнении, ФайлКОтправке);
	КонецЕсли;
	
	Возврат Результат;
КонецФункции

Функция УдалитьЗадачуИзТрекера(НомерЗадачи, ОтчетоВыполнении) Экспорт
	
	//1. получить связанные задачи, если они есть необходимо эти связи удалить
	ДанныеСвязей = ПолучитьДанныеРесурсаИзТрекера("/issues/"+Формат(НомерЗадачи, "ЧГ=0")+"/relations", ОтчетОВыполнении);
	
	Если ДанныеСвязей <> Неопределено Тогда  // Необходимо удалить связи
		Для Каждого Связь Из ДанныеСвязей["relations"] Цикл
			ОтправитьДанныеВТрекер("DELETE", "/relations/"+Связь.id, ОтчетоВыполнении);		
			Если ОтчетоВыполнении.ЕстьОшибки Тогда
				Возврат Ложь;		
			КонецЕсли;	
		КонецЦикла;	
	...
	//2. удаление задачи
	ОтправитьДанныеВТрекер("DELETE", "/issues/"+Формат(НомерЗадачи, "ЧГ=0"), ОтчетоВыполнении);		
	Если ОтчетоВыполнении.ЕстьОшибки Тогда
		Возврат Ложь;		
	КонецЕсли;	
...
Функция СформироватьСтрутуруДанных(Редактирование=Ложь)
	
	Задача = Новый Структура;
	
	ПоляЗадачи = Новый Структура;
	ПоляЗадачи.Вставить("project_id",             ПолучитьИдентификаторПараметра(ЗадачаПроект, "projects"));
	ПоляЗадачи.Вставить("parent_issue_id",    	  ?(ЗадачаНомерРодителя = 0, Неопределено, ЗадачаНомерРодителя)); 	
	ПоляЗадачи.Вставить("tracker_id",             ПолучитьИдентификаторПараметра(ЗадачаТрекер, "trackers"));
	ПоляЗадачи.Вставить("status_id",              ПолучитьИдентификаторПараметра(ЗадачаСтатус, "issue_statuses"));
	ПоляЗадачи.Вставить("priority_id",            ПолучитьИдентификаторПараметра(ЗадачаПриоритет, "issue_priorities"));
	ПоляЗадачи.Вставить("subject",                ЗадачаТема);
	ПоляЗадачи.Вставить("description",            ЗадачаОписание);
	ПоляЗадачи.Вставить("estimated_hours",        ЗадачаОценкаВремени); 
	ПоляЗадачи.Вставить("start_date",             Формат(ЗадачаНачалоВыполнения, "ДФ=yyyy-MM-dd")); 
	ПоляЗадачи.Вставить("due_date",               Формат(ЗадачаКонецВыполнения, "ДФ=yyyy-MM-dd")); 
	ПоляЗадачи.Вставить("assigned_to_id",         ПолучитьИдентификаторПараметра(ЗадачаИсполнитель, "memberships")); 
	
	Если Редактирование И ЗадачаКомментарии <> "" Тогда
		ПоляЗадачи.Вставить("notes", ЗадачаКомментарии); 
	КонецЕсли;
...
Функция СформироватьСтрутуруДанныхВремя()
...
Функция ПолучитьMIMEТип(Файл)
	//:	Файл = Новый Файл("");
	
	Если Файл.Расширение = ".jpg" Или Файл.Расширение = ".png" 
		Или Файл.Расширение = ".bmp" Или Файл.Расширение = ".gif" Или Файл.Расширение = ".jpeg" Тогда // image/*
		
		MIMEТип = "image/"+Прав(Файл.Расширение, СтрДлина(Файл.Расширение)-1);
		
	ИначеЕсли Файл.Расширение = ".txt" Или Файл.Расширение = ".css" 
		Или Файл.Расширение = ".csv" Или Файл.Расширение = ".html" Или Файл.Расширение = ".xml" Тогда // text/*
		
		MIMEТип = "text/"+Прав(Файл.Расширение, СтрДлина(Файл.Расширение)-1);
	Иначе // file/*
		
		MIMEТип = "file/"+Прав(Файл.Расширение, СтрДлина(Файл.Расширение)-1);
	КонецЕсли;
...
Функция ОтправитьДанныеВТрекер(Метод, Ресурс, ОтчетоВыполнении, ФайлКотправке = Неопределено)
	...
	Возврат ОбработатьРезультатОтвета(HTTPОтвет, ОтчетоВыполнении);
КонецФункции


Функция ОтправитьЗапросВТрекер(СтруктураЗапроса, ОтчетОВыполении)
	
	Если НЕ ЗначениеЗаполнено(APIkey) Тогда
		ОтчетОВыполении.ЕстьОшибки = Истина;
		ОтчетОВыполении.ТекстОшибки = "Не указан API key для доступа к трекеру Redmine!";
	...
	КонецЕсли;
	мHTTPСоединение = Новый HTTPСоединение("redmine.gisko.org", 80, "asimonyan", "rm@11simonyan.org");
	
	Параметры = ОбязательныйПараметр + СтруктураЗапроса.Параметры;
	
	Попытка
		Если СтруктураЗапроса.Метод = "GET" Тогда
			HTTPОтвет = мHTTPСоединение.Получить(Новый HTTPЗапрос(СтруктураЗапроса.АдресЗапроса+Параметры));
		Иначе
			Если Найти(СтруктураЗапроса.АдресЗапроса, "uploads") > 0 Тогда
			...
			HTTPЗапрос = Новый HTTPЗапрос(СтруктураЗапроса.АдресЗапроса);
	...
	Исключение
		ОтчетОВыполении.ЕстьОшибки = Истина;
		ОтчетОВыполении.ТекстОшибки = "Не удалось соединится с сервером Redmine!";
		Возврат Неопределено;
	КонецПопытки;
...
Функция ОбработатьРезультатОтвета(HTTPОтвет, ОтчетОВыполнении)
	Перем Результат;	
	...
	Если HTTPОтвет <> Неопределено Тогда
		...
		Результат = РазобратьОтветВФорматеJSON(HTTPОтвет.ПолучитьТелоКакСтроку());
		...
		Если Результат = Неопределено Тогда
			...
			ОтчетоВыполнении.ТекстОшибки = "Ошибка выполнения запроса. Код состояния:" + HTTPОтвет.КодСостояния;
			ОтчетоВыполнении.ТекстОшибки = ОтчетоВыполнении.ТекстОшибки + Символы.ПС + "Результат ответа сервера неопределен, проверьте настройки!";
			Возврат Неопределено;
		КонецЕсли;
...
Функция ЗадачаУжеДобавленаВТрекер(СтруктураДанных, ОтчетоВыполнении)
...
Функция СформироватьФайлКОтправкеJSON(СтруктураДанных, ОтчетОВыполнении)
	
	ФайлКОтправке = ПолучитьИмяВременногоФайла("json");
	
	СтрокаJSON = асЗаписатьJSON(СтруктураДанных, Ложь);
	
	// Запись в Файл необходимоа для удаления управляющего BOM символа, т.к. присутствие такового вызывает исключение в трекере
	...
	Возврат ФайлКОтправке;
КонецФункции

Функция РазобратьОтветВФорматеJSON(СтрокаJSON)
	Перем Результат;
...
Функция ВсеРеквизитыЗаполнены(СтруктураПолей, ИмяДействия = "", Сообщать = Истина)
	
	ВсеЗаполнены = Истина;
	Для каждого Поле из СтруктураПолей Цикл
		Если НЕ ЗначениеЗаполнено(ЭтотОбъект[Поле.Ключ].Значение) Тогда
			Если Сообщать Тогда 
				#Если Клиент Тогда
...
Функция ВосстановитьНастройкиФормы(ФормаОбработки, ВсеНастройки = Истина) Экспорт
	...
	Если ВсеНастройки Тогда
		...
		Если Не НастройкиФормы.Свойство("ПоказатьОписание") Тогда
			НастройкиФормы.Вставить("ПоказатьОписание", Истина);
		КонецЕсли;
	КонецЕсли;
...
Процедура СохранитьНастройкиФормы(ФормаОбработки) Экспорт
	...
	НастройкиФормы.Вставить("ПоказатьОписание"         , ЭлементыФормы.КоманднаяПанельСписокЗадач.Кнопки.ПоказатьСкрытьОписание.Пометка);
...
Процедура ЗаполнитьСписокВыбораЭлементаФормы(Форма, ИмяПоля, Ресурс, Ключ="", ДопМассив=Неопределено) Экспорт
	...
	Если Данные = Неопределено Тогда
		...
		Данные = ПолучитьДанныеРесурсаИзТрекера(Ресурс, ОтчетОВыполнении);
...
// Feature #2078: Настроить сервис GitHub для автоматического обновления задач в Redmine
