...
Процедура КоманднаяПанельСписокЗадачПерейти(Кнопка)
	...
	СтрокаПорт = ?(Порт = 80 ИЛИ Порт = 443, "", ":" + Формат(Порт, "ЧГ=0"));
	Протокол = ?(Порт = 443, "http://", "https://");
	...
	Если Найти(Кнопка.Имя, "СписокЗадач") Тогда
		...
		Если Не ТекСтрока = Неопределено Тогда
			...
			Если СтрокаДерева <> Неопределено И СтрокаДерева.Идентификатор <> 0 Тогда
				...
				ЗапуститьПриложение(Протокол + Сервер + СтрокаПорт + "/projects/p"+Формат(ИдПроекта-1, "ЧЦ=3; ЧВН=; ЧГ=0") + "/issues");
			КонецЕсли;
		КонецЕсли;
	Иначе
		ТекСтрока = ЭлементыФормы.СписокЗадач.ТекущиеДанные;
		Если Не ТекСтрока = Неопределено Тогда
			ЗапуститьПриложение(Протокол + Сервер + СтрокаПорт + "/issues/"+Формат(ТекСтрока.Идентификатор, "ЧГ=0"));
		КонецЕсли;
	КонецЕсли;
...
Процедура ПроверкаСоединенияНажатие(Элемент)
	...
	Если НЕ ОтчетоВыполнении.ЕстьОшибки Тогда
		...
		УспешнаяИнициализация = Инициализация(Истина, ?(Проект<>"", Проект, Неопределено));
	Иначе
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если Источник = "ФормаЗадачи" И ИмяСобытия = "ОбновитьСписокЗадач" Тогда
