...
Функция ПолучитьДанныеРесурсаИзТрекера(Ресурс, ОтчетоВыполнении, Параметры = "", Кэшировать = Истина) Экспорт
	...
	Если Не ОтчетоВыполнении.ЕстьОшибки И Результат.Свойство("total_count") И Догружать Тогда
		Пока Результат["total_count"] > Результат[Ресурс].Количество() Цикл
			...
			ЧастьДанных = ОтправитьЗапросВтрекер(СтруктураЗапросаСледЧастиДанных, ОтчетоВыполнении);
...
Функция ОтправитьЗапросВТрекер(СтруктураЗапроса, ОтчетоВыполнении)
	...
	Возврат ОбработатьРезультатОтвета(HTTPОтвет, СтруктураЗапроса, ОтчетоВыполнении); 
КонецФункции

Функция ОбработатьРезультатОтвета(HTTPОтвет, СтруктураЗапроса, ОтчетОВыполнении)
	Перем Результат;	
