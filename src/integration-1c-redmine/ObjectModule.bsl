...
Функция ОбновитьЗадачуВТрекере() Экспорт
	...
	// Подготовить файл к отправке в трекер
	ПреобразоватьСтруктуруВ_JSON(СформироватьСтруктуруЗадачи(Истина), ФайлКОтправке, СтрокаКОтправке);
	...
	Попытка
		...
		Иначе
			...
			Если НЕ СтруктураЗапроса.СтрокаКОтправке = Неопределено  Тогда
				HTTPЗапрос.УстановитьТелоИзСтроки(СтруктураЗапроса.СтрокаКОтправке);
			КонецЕсли;
